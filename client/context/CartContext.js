...
export function CartProvider({ children }) {
    ...
    const addToCart = (product, quantity = 1) => {
        ...
        setItems((current) => {
            ...
            if (existing) {
                return current.map((item) => item === existing ? { ...item, quantity: Math.min(stock, item.quantity + quantity) } : item);
            }
            return [...current, { ...product, id, quantity: Math.min(stock, Math.max(1, quantity)) }];
        });
    };
    ...
    const updateQuantity = (id, quantity) => {
        setItems((current) => current.map((item) => {
            ...
            return { ...item, quantity: Math.min(stock, next) };
    ...
    return <CartContext.Provider value={{ items, addToCart, updateQuantity, removeFromCart, clearCart, cartCount }}>{children}</CartContext.Provider>;
}
