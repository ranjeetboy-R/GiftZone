...
export default function ProductCard({ product }) {
  ...
  return <article className="group overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm transition duration-300 hover:-translate-y-1 hover:shadow-xl">
    <div className="relative flex items-center justify-center overflow-hidden bg-[#fff8f4]">
      <Link href={`/product/${slug}`} className="relative flex w-full h-40 items-center justify-center">
        <Image src={image} fill alt={product.name} className="h-full w-full object-cover transition duration-500 group-hover:scale-105" />
      </Link>
    ...
    <div className="p-4">
      ...
      <Link href={`/product/${slug}`}>
        <h3 className="mt-1 min-h-10 text-sm font-bold leading-5 hover:text-[#c92532]">{product.name}</h3>
      </Link>
      <div className="mt-2 flex items-center gap-1 text-xs">
        <span className="flex items-center gap-1 text-amber-500">
          <Star size={13} fill="currentColor" /> {product.rating || 0}</span>
